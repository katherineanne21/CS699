...
```{r} 
# Accident data has already been read in

# Remove no injury observations
acc_data2 <- subset(acc_data, MAX_SEV != "no-injury")
...
score of 0.047. This is most likely because our positive occurances are very rare.
```
...
undersampling would decrease the size to only 16 observations if we weight the
classes equally. When there are very few positive cases (only 8 in this case)
then it is a bad idea to undersample.
```
...
WEATHER_adverse, RushHour, and INT_HWY.
```
